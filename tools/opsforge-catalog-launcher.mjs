...
// URL 经 new URL 校验 host 必须 127.0.0.1 或 <owner>.github.io（能力广场白名单，后缀匹配）；
// 失败只打印业务指引不报错。全 argv 形式（无 shell:true）；detached+unref 让子进程脱离父进程生命周期。
import { spawn } from 'node:child_process';

const ALLOWED_HOST_SUFFIXES = ['.github.io'];
const ALLOWED_HOSTS_EXACT = ['127.0.0.1'];

/**
 * openInBrowser(url) — 跨平台 spawn 浏览器.
 * @param {string} url  必须 http://127.0.0.1:<port>/... 或 https://<owner>.github.io/... 形式
...
export function openInBrowser(url, opts = {}) {
  ...
  const allowed = ALLOWED_HOSTS_EXACT.includes(host)
    || ALLOWED_HOST_SUFFIXES.some((s) => host.endsWith(s) && host.length > s.length);
  if (!allowed) {
