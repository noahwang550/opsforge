...
- **CI workflows** — `ci/github-actions/{validate,test,release}.yml` on PR (5-platform matrix; Node 22 via `.nvmrc`); `ci/github-actions/re-eval.yml` weekly cron (re-eval + effectiveness<70% degraded); `ci/guardrails.yml` path-guard covers `adapters/*/` + all tools; `CODEOWNERS` extended to `packs/opsforge-meta/` + Phase 3.6 tools.
