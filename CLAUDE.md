...
- **CI workflows** — `.github/workflows/{validate,test,release}.yml` on PR (5-platform matrix; Node 22 via `.nvmrc`); `.github/workflows/re-eval.yml` weekly cron (re-eval + effectiveness<70% degraded); `ci/guardrails.yml` path-guard covers `adapters/*/` + all tools (NOTE: guardrails.yml currently lives in `ci/` and is NOT wired into `.github/workflows/` — see open issue); `CODEOWNERS` extended to `packs/opsforge-meta/` + Phase 3.6 tools.
